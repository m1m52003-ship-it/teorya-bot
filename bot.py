...
# =========================
# Tesseract OCR
# ملاحظة: قد لا يعمل OCR على Render بدون إعدادات إضافية
# =========================
...
async def handle_photo_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            "يفضل إرسال رقم السؤال أو جزء من نصه."
        )
        return
    finally:
        try:
...
def main():
    if not TOKEN:
        raise ValueError("TOKEN غير موجود. أضفه في Render داخل Environment Variables")
    ...
    if webhook_url_env:
        ...
        print("✅ تشغيل محلي بنظام polling")
        app.run_polling()
        return

    print("✅ تشغيل البوت على Render بنظام Webhook")
